...
lastUpdated: 'April 3, 2026 at 6:04 AM EDT'
---
...
- `/rewind` — Open a timeline picker and roll back to any earlier point in conversation history. Press double-Esc to open the same picker. [Release: v1.0.13][release-1-0-13]
...
- `/share [file|gist|html] [PATH]` — Export the session to Markdown, a secret gist, or self-contained interactive HTML. [Docs: slash commands][slash-commands] [Release: v1.0.15][release-1-0-15] [Blog: slash commands][blog-slash-commands]
...
- `/mcp [show|add|edit|delete|disable|enable|auth] [SERVER-NAME]` — Manage MCP servers and OAuth authentication for them. [Docs: slash commands][slash-commands] [Release: v1.0.15][release-1-0-15] [Blog: slash commands][blog-slash-commands]
...
- `postToolUseFailure` [Release: v1.0.15][release-1-0-15]
...
- `postToolUseFailure` handles tool errors, while `postToolUse` now runs only after successful tool calls. [Release: v1.0.15][release-1-0-15]
- `PermissionRequest` hooks can programmatically approve or deny tool permission requests. [Release: v1.0.16][release-1-0-16]
...
Recent releases also bundle built-in skills with the CLI itself, starting with a guide for customizing Copilot cloud agent's environment. You can inspect or invoke them with `/skills`. [Docs: slash commands][slash-commands] [Release: v1.0.17][release-1-0-17]
...
Recent official releases added or improved several user-facing CLI features. [Release: v1.0.17][release-1-0-17]

- Built-in skills ship with the CLI, starting with a guide for customizing Copilot cloud agent's environment. [Release: v1.0.17][release-1-0-17]
- `PermissionRequest` hooks can script approval or denial of tool permission prompts. [Release: v1.0.16][release-1-0-16]
- `/mcp auth` adds re-authentication UI for OAuth-backed MCP servers, including account switching support. [Release: v1.0.15][release-1-0-15]
- `/share html` exports sessions and research reports as self-contained interactive HTML files. [Release: v1.0.15][release-1-0-15]
- `postToolUseFailure` handles tool errors, and `postToolUse` now runs only after successful tool calls. [Release: v1.0.15][release-1-0-15]
- `/rewind` and double-Esc open a timeline picker that can roll back to any earlier point in conversation history. [Release: v1.0.13][release-1-0-13]
